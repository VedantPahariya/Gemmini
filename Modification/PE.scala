...
class MacUnit[T <: Data](inputType: T, weightType: T, cType: T, dType: T, has_ternary_weights: Boolean = false) (implicit ev: Arithmetic[T]) extends Module {
  import ev._
  ...
  if (has_ternary_weights) {
    // BitNet ternary weights: {-1, 0, 1}
    // Use bit patterns to identify ternary values:
    // 0 = 0x00, 1 = 0x01, -1 = 0xFF (int8 two's complement)
    val weight_bits = io.in_b.asTypeOf(UInt(8.W))
    val activation = io.in_a
    val accumulator = io.in_c
    
    // Check bit patterns for ternary values
    val is_plus_one = weight_bits === 1.U(8.W)
    val is_minus_one = weight_bits === 255.U(8.W)  // 0xFF = -1 in two's complement
    val is_zero = weight_bits === 0.U(8.W)
    
    // Compute result based on weight value
    val result = Wire(dType)
    when (is_plus_one) {
      result := (accumulator + activation).asTypeOf(dType)
    }.elsewhen (is_minus_one) {
      result := (accumulator - activation).asTypeOf(dType)
    }.otherwise {
      result := accumulator.asTypeOf(dType)
    }
    io.out_d := result
  } else {
    // Standard MAC operation for general weights
...
/**
  ...
  */
class PE[T <: Data](inputType: T, weightType: T, outputType: T, accType: T, df: Dataflow.Value, max_simultaneous_matmuls: Int, has_ternary_weights: Boolean = false)
  ...
  val mac_unit = Module(new MacUnit(inputType, weightType,
    if (df == Dataflow.WS) outputType else accType, outputType, has_ternary_weights))
