...
object GemminiConfigs {
  ...
  val leanConfig = defaultConfig.copy(dataflow=Dataflow.BOTH, max_in_flight_mem_reqs = 64, acc_read_full_width = false, ex_read_from_acc = false, ex_write_to_spad = true, hardcode_d_to_garbage_addr = true)

  val leanPrintfConfig = defaultConfig.copy(dataflow=Dataflow.BOTH, max_in_flight_mem_reqs = 64, acc_read_full_width = false, ex_read_from_acc = false, ex_write_to_spad = true, hardcode_d_to_garbage_addr = true, use_firesim_simulation_counters=true)

  // BitNet configuration optimized for ternary weights {-1, 0, 1}
  // Note: weightType kept at 8 bits to satisfy inputType.getWidth == weightType.getWidth requirement
  // Ternary weights {-1, 0, 1} will use only 2 bits of the 8-bit representation
  val bitnetConfig = defaultConfig.copy(
    dataflow = Dataflow.BOTH,              // Weight-stationary is optimal for ternary
    has_ternary_weights = true,          // Enable ternary weight optimization in PE
    sp_capacity = CapacityInKilobytes(128),
...
/**
 * Mixin which sets BitNet-optimized parameters for ternary weight acceleration.
 */
class BitNetGemminiConfig[T <: Data : Arithmetic, U <: Data, V <: Data](
  gemminiConfig: GemminiArrayConfig[T,U,V] = GemminiConfigs.bitnetConfig
) extends Config((site, here, up) => {
  case BuildRoCC => up(BuildRoCC) ++ Seq(
    (p: Parameters) => {
      implicit val q = p
