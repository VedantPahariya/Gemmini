...
/**
  ...
  */
class Tile[T <: Data](inputType: T, weightType: T, outputType: T, accType: T, df: Dataflow.Value, tree_reduction: Boolean, max_simultaneous_matmuls: Int, val rows: Int, val columns: Int, has_ternary_weights: Boolean = false)(implicit ev: Arithmetic[T]) extends Module {
  ...
  val tile = Seq.fill(rows, columns)(Module(new PE(inputType, weightType, outputType, accType, df, max_simultaneous_matmuls, has_ternary_weights)))
