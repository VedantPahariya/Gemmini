...
class MeshWithDelays[T <: Data: Arithmetic, U <: TagQueueTag with Data]
  (val inputType: T,  val weightType: T, val outputType: T, accType: T,
   ...
   leftBanks: Int, upBanks: Int, outBanks: Int = 1, n_simultaneous_matmuls: Int = -1, has_ternary_weights: Boolean = false)
  extends Module {
  ...
  val mesh = Module(new Mesh(inputType, weightType, outputType, accType, df, tree_reduction, tile_latency, max_simultaneous_matmuls, output_delay, tileRows, tileColumns, meshRows, meshColumns, has_ternary_weights))
